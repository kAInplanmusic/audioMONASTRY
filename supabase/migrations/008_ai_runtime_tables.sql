-- ============================================================================
-- Konsolidierung DB-P2-001 (2026-09-23) - HERKUNFT: database/ai_migration_001.sql
-- ============================================================================
-- Warum diese Datei hier liegt:
-- Diese Objekte fehlten im angewandten supabase-Satz komplett: mcp_audit_events
-- sowie die AI-Laufzeittabellen aus 001. Ohne sie kann server/routes/aiRoutes.ts
-- (auditMcp) nicht schreiben - der RLS-Fix waere auf einer per supabase
-- provisionierten Datenbank ins Leere gelaufen.
--
...
-- ============================================================================
-- ----------------------------------------------------------------------------
-- KORREKTUR bei der Konsolidierung (2026-09-23):
-- Die Vorlage database/ai_migration_001.sql legte fuer die AI-/MCP-Tabellen
-- anon-SELECT-Policies an. Woertlich kopiert haetten sie den Fix aus
-- 007_rls_harden_anon_read.sql (RC1-004) wieder aufgehoben, denn 008 laeuft
-- alphabetisch NACH 007. Deshalb ist aus jedem "create policy ... to anon"
-- hier ein "drop policy if exists" geworden: die Freigabe wird nicht nur nicht
-- angelegt, sondern einer Datenbank mit altem Stand 001 aktiv entzogen.
-- Grundlage des Entzugs: nur aiPersistence.ts beruehrt diese Tabellen, und zwar
-- serverseitig mit service_role; 0 Treffer im Client-Bundle.
-- ----------------------------------------------------------------------------
...
drop policy if exists "anon_read_ai_migrations" on public.ai_migrations;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
drop policy if exists "anon_read_ai_sessions" on public.ai_sessions;
drop policy if exists "anon_read_ai_sessions" on public.ai_sessions;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
drop policy if exists "anon_read_ai_jobs" on public.ai_jobs;
drop policy if exists "anon_read_ai_jobs" on public.ai_jobs;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
drop policy if exists "anon_read_ai_model_usage" on public.ai_model_usage;
drop policy if exists "anon_read_ai_model_usage" on public.ai_model_usage;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
drop policy if exists "anon_read_ai_errors" on public.ai_errors;
drop policy if exists "anon_read_ai_errors" on public.ai_errors;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
drop policy if exists "anon_read_ai_cost" on public.ai_cost_estimates;
drop policy if exists "anon_read_ai_cost" on public.ai_cost_estimates;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
drop policy if exists "anon_read_mcp_audit" on public.mcp_audit_events;
drop policy if exists "anon_read_mcp_audit" on public.mcp_audit_events;   -- RC1-004: bewusst KEIN anon-Lesen (siehe 007)
