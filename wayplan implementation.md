# Interne Entwicklungsdokumentation – Strang B (Implementierung)

## Rolle dieses Strangs (Implementierung)

Du agierst als ausführender Entwickler und setzt die Spezifikation nach
Prüfung auf Sinn und Mehrwert eigenständig um.
- **Kein externer Code:** Es wird nicht auf analysierte Original-Codebasen
  zugegriffen und kein Code aus existierenden DAW-Projekten kopiert.
- **Eigenständige Architektur:** Rust/WASM, TypeScript und UI werden
  vollständig "from scratch" auf Basis der abstrakten Vorgaben geschrieben.
- **Dokumentation:** Jeder architektonische Beschluss, jedes Modul und jeder
  Code-Block wird mit Zeitstempel in dieser internen Dokumentation festgehalten.
- **Verhaltenskodex:** Bei Unklarheiten in der Spezifikation wird nicht
  geraten, sondern eine Präzisierung angefordert.
